...
const FAVORITE_PREVIEW = [
    { id: 'fav-1', name: 'Cervisia IPA', style: 'Hazy IPA', location: 'Oslo, NO', rating: '4.7' },
    { id: 'fav-2', name: 'Nordic Pils', style: 'Crisp Pilsner', location: 'Bergen, NO', rating: '4.5' },
    { id: 'fav-3', name: 'Arctic Stout', style: 'Imperial Stout', location: 'Tromsø, NO', rating: '4.8' },
];

const INTEREST_TAGS = ['Hazy IPAs', 'Barrel aged', 'Low ABV', 'Seasonal releases', 'Local taps'];

const SettingsScreen = ({ navigation }) => {
    const { user, logout } = useAuth();

    const displayName = useMemo(
        () => (user?.displayName && user.displayName.trim().length > 0 ? user.displayName.trim() : 'Beer Explorer'),
        [user?.displayName],
    );
    ...
    const handleSupport = () => {
        Alert.alert('Support', 'Send us a message at support@cervisia.app and we will get right back to you.');
    };
        ...
        Alert.alert('Sign out', 'Are you sure you want to sign out?', [
            { text: 'Cancel', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    ...
                    if (!result.success) {
                        Alert.alert('Oops', 'We could not sign you out. Try again in a moment.');
                    }
                },
            },
        ]);
        ...
        <View style={ProfileScreenStyle.safeArea}>
            ...
            >
                ...
                <View style={ProfileScreenStyle.actionGrid}>
                    <TouchableOpacity style={ProfileScreenStyle.actionButton} onPress={handleEditProfile}>
                        <View style={ProfileScreenStyle.actionLabelGroup}>
                            <Text style={ProfileScreenStyle.actionLabel}>Edit profile</Text>
                            <Text style={ProfileScreenStyle.actionSubtitle}>Update your bio and avatar</Text>
                        </View>
                        <View style={ProfileScreenStyle.actionIconContainer}>
                            <Ionicons name="create-outline" size={22} color={Colors.buttonText} />
                        </View>
                    </TouchableOpacity>

                    <TouchableOpacity style={ProfileScreenStyle.actionButton} onPress={handleAccountSettings}>
                        <View style={ProfileScreenStyle.actionLabelGroup}>
                            <Text style={ProfileScreenStyle.actionLabel}>Account settings</Text>
                            <Text style={ProfileScreenStyle.actionSubtitle}>Security & notifications</Text>
                        </View>
                        <View style={ProfileScreenStyle.actionIconContainer}>
                            <Ionicons name="settings-outline" size={22} color={Colors.buttonText} />
                        </View>
                    </TouchableOpacity>

                    <TouchableOpacity style={ProfileScreenStyle.actionButton} onPress={handleFavorites}>
                        <View style={ProfileScreenStyle.actionLabelGroup}>
                            <Text style={ProfileScreenStyle.actionLabel}>My favorites</Text>
                            <Text style={ProfileScreenStyle.actionSubtitle}>See saved beers & spots</Text>
                        </View>
                        <View
                            style={[
                    ...
                    <TouchableOpacity style={ProfileScreenStyle.actionButton} onPress={handleAppDetails}>
                        <View style={ProfileScreenStyle.actionLabelGroup}>
                            <Text style={ProfileScreenStyle.actionLabel}>App details</Text>
                            <Text style={ProfileScreenStyle.actionSubtitle}>Version notes & roadmap</Text>
                        </View>
                        <View
                            style={[
                                ProfileScreenStyle.actionIconContainer,
                    ...
                    {FAVORITE_PREVIEW.map((item, index) => (
                        <View
                            ...
                            style={[
                                ...
                                index === FAVORITE_PREVIEW.length - 1 && ProfileScreenStyle.favoriteItemLast,
                            ]}
                        >
                ...
                <View style={ProfileScreenStyle.supportCard}>
                    ...
                    <Text style={ProfileScreenStyle.supportText}>
                        Reach out if you have questions about your account, data privacy, or the next beer
                        recommendation. Our team typically replies within a day.
                    </Text>
                    <View style={ProfileScreenStyle.chipRow}>
                        <TouchableOpacity style={ProfileScreenStyle.chip} onPress={handleSupport}>
                            <Text style={ProfileScreenStyle.chipText}>Contact support</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={ProfileScreenStyle.chip} onPress={handleAppDetails}>
                            <Text style={ProfileScreenStyle.chipText}>Release notes</Text>
                        </TouchableOpacity>
                    </View>
                    <TouchableOpacity style={ProfileScreenStyle.signOutButton} onPress={handleLogout}>
